...
from RecordandGameTreeAI import StepRecordChessBoard
import threading
...
class CheckerBoardView(tk.Tk, object):
    ...
    def build_board(self):
        ...
        self.board = Canvas(self, bg="#FFFFF0", width=700, height=630)
        ...
        if self.is_our_side:    # 仅本方回合可以下棋
            ...
            if 590 > mouse_x > 30 and 590 > mouse_y > 30:
                ...
                if result == 1:
                    ...
                    tk.messagebox.showinfo('提示', '本方获胜！').place(240, 550)
                    self.endgame_result()
                else:
                    pass

            callback_human = threading.Thread(target=self.callback_other)
    ...
    # 用于获取对方的数据
    def other_human(self, game_data):
        head, win, piece_num, str_x, str_y = game_data.split('#')

        if win == '1':
            # 解除鼠标左键绑定
            self.unbind('<Button-1>')
            tk.messagebox.showinfo('提示', '对方获胜！').place(240, 550)
            self.endgame_result()
        else:
            pass
    ...
    def callback_other(self):
        game_data = self.tcp_control.get_game_data()
        while True:
            game_data = self.tcp_control.get_game_data()
            if game_data[0:1] == '3':
                break
        self.other_human(game_data)
