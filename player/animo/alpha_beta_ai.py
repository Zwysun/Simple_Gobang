...
class Gomoku:
    ...
    def __init__(self, max_search_steps = 3):
        self.g_map = [[0 for y in range(15)] for x in range(15)]  # 当前的棋盘
        self.cur_step = 0  # 步数
        self.max_search_steps = max_search_steps  # 最远搜索2回合之后
...
class Node:
    ...
    def calc_score(self):
        ...
        # 2. 判断白色和黑色的四子的数目（需要保证：不是已经被堵死的四子）
        white_4_num = 0
        black_4_num = 0
        for x in range(11):
            for y in range(15):
                black_cnt = sum([self.game.g_map[x][y] == 1, self.game.g_map[x + 1][y] == 1, self.game.g_map[x + 2][y] == 1, self.game.g_map[x + 3][y] == 1, self.game.g_map[x + 4][y] == 1])
                white_cnt = sum([self.game.g_map[x][y] == 2, self.game.g_map[x + 1][y] == 2, self.game.g_map[x + 2][y] == 2, self.game.g_map[x + 3][y] == 2, self.game.g_map[x + 4][y] == 2])
                if black_cnt == 4 and white_cnt == 0:
                    black_4_num += 1
                if white_cnt == 4 and black_cnt == 0:
                    white_4_num += 1
        for x in range(15):
            for y in range(11):
                black_cnt = sum([self.game.g_map[x][y] == 1, self.game.g_map[x][y + 1] == 1, self.game.g_map[x][y + 2] == 1, self.game.g_map[x][y + 3] == 1, self.game.g_map[x][y + 4] == 1])
                white_cnt = sum([self.game.g_map[x][y] == 2, self.game.g_map[x][y + 1] == 2, self.game.g_map[x][y + 2] == 2, self.game.g_map[x][y + 3] == 2, self.game.g_map[x][y + 4] == 2])
                if black_cnt == 4 and white_cnt == 0:
                    black_4_num += 1
                if white_cnt == 4 and black_cnt == 0:
                    white_4_num += 1
        for x in range(11):
            for y in range(11):
                black_cnt = sum([self.game.g_map[x][y] == 1, self.game.g_map[x + 1][y + 1] == 1, self.game.g_map[x + 2][y + 2] == 1, self.game.g_map[x + 3][y + 3] == 1, self.game.g_map[x + 4][y + 4] == 1])
                white_cnt = sum([self.game.g_map[x][y] == 2, self.game.g_map[x + 1][y + 1] == 2, self.game.g_map[x + 2][y + 2] == 2, self.game.g_map[x + 3][y + 3] == 2, self.game.g_map[x + 4][y + 4] == 2])
                if black_cnt == 4 and white_cnt == 0:
                    black_4_num += 1
                if white_cnt == 4 and black_cnt == 0:
                    white_4_num += 1
        for x in range(11):
            for y in range(11):
                black_cnt = sum([self.game.g_map[x + 4][y] == 1, self.game.g_map[x + 3][y + 1] == 1, self.game.g_map[x + 2][y + 2] == 1, self.game.g_map[x + 1][y + 3] == 1, self.game.g_map[x][y + 4] == 1])
                white_cnt = sum([self.game.g_map[x + 4][y] == 2, self.game.g_map[x + 3][y + 1] == 2, self.game.g_map[x + 2][y + 2] == 2, self.game.g_map[x + 1][y + 3] == 2, self.game.g_map[x][y + 4] == 2])
                if black_cnt == 4 and white_cnt == 0:
                    black_4_num += 1
                if white_cnt == 4 and black_cnt == 0:
                    white_4_num += 1
        ...
        if self.player_first:
            if self.depth % 2 == 0:  # 该轮到黑棋出了
                if black_4_num >= 2:
                    return -90
                elif white_4_num >= 2 and black_4_num == 0:
                    return 90
            else:  # 该轮到白棋出了
                if white_4_num >= 2:
                    return 90
                elif black_4_num >= 2 and white_4_num == 0:
                    return -90
        else:
            if self.depth % 2 == 0:  # 该轮到白棋出了
                if white_4_num >= 2:
                    return 90
                elif black_4_num >= 2 and white_4_num == 0:
                    return -90
        ...
        if self.player_first:
            if self.depth % 2 == 0:  # 该轮到玩家出了
                if black_4_num and black_3d_num >= 1:
                    return -80
                elif white_4_num and white_3d_num >= 1:
                    return 80
            else:  # 该轮到电脑出了
                if white_4_num and white_3d_num >= 1:
                    return 80
                elif black_4_num and black_3d_num >= 1:
                    return -80
        else:
                    ...
                    for x0 in range(x - 1, x + 2):
                        for y0 in range(y - 1, y + 2):
                            if 0 <= x0 <= 14 and 0 <= y0 <= 14 and self.game.g_map[x0][y0] != 0:
                                around_cnt += 1
                    black_score_num += score_by_num_around[around_cnt] - abs(x - 7) - abs(y - 7)
                    black_cnt += 1
                if self.game.g_map[x][y] == 2:
                    ...
                    for x0 in range(x - 1, x + 2):
                        for y0 in range(y - 1, y + 2):
                            if 0 <= x0 <= 14 and 0 <= y0 <= 14 and self.game.g_map[x0][y0] != 0:
                                around_cnt += 1
                    white_score_num += score_by_num_around[around_cnt] - abs(x - 7) - abs(y - 7)
                    white_cnt += 1
        if white_cnt == 0 or black_cnt == 0:
            return 0
        score = white_score_num / white_cnt - black_score_num / black_cnt
        return score
