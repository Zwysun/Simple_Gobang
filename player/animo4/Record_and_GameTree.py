...
class StepRecordChessBoard(object):
    ...
    def __init__(self, table):
        ...
        self.records = [[None for i in range(15)] for j in range(15)]  # 棋盘初始化为19*19的0矩阵

        #
        self.playerjudger = None
        self.table_2d = table.table_2d[1:16, 1:16]
        self.t1 = None
        ...
        # 数组记录
        self.white_index = []  # 记录白棋坐标数组
        self.black_index = []  # 记录黑棋坐标数组
        self.all_index = []  # 记录所有棋子坐标数组
        ...
        self.col = 15  # 占位暂定
        self.row = 15  # 占位暂定
        self.depth = 3  # 搜索深度，即AI的远见程度，搜索深度为3即AI能看3步布局
        self.strategy = 1.1  # 策略系数，大于1为进攻型策略，小于1为保守型策略
        self.shape_score = [(100, (0, 1, 1, 0, 0)),  # 各种棋型的评估分数（score， shape）
                            (100, (0, 0, 1, 1, 0)),
                            (1000, (1, 1, 0, 1, 0)),
                            (1000, (0, 0, 1, 1, 1)),
                            (1000, (1, 1, 1, 0, 0)),
    ...
    def check_down(self, x, y):
        ...
        if self.has_record(x, y) and self.has_record(x + 1, y - 1) and self.has_record(x + 2,
            ...
            elif self.records[x][y].color == 2 and self.records[x + 1][y - 1].color == 2 and self.records[x + 2][
                        y - 2].color == 2 and self.records[x + 3][y - 3].color == 2 and self.records[x + 4][
                        y - 4].color == 2:
                return 0
            return -1
    ...
    def insert_position_list(self, color, position):
        if color == 1:
            self.white_index.append(position)
        else:
            self.black_index.append(position)
        self.all_index.append(position)
    ...
    def evaluation(self):
        # 判断是哪一方进行判定
        if self.color == 1:
            my_list = self.white_index
            enemy_list = self.black_index
        else:
            my_list = self.black_index
            enemy_list = self.white_index
    ...
    def has_neighbor(self, position):
        ...
        for i in range(-1, 2):
            for j in range(-1, 2):
                if i == 0 and j == 0:
                    continue
                if (position[0] + i, position[1] + j) in self.all_index:
                    return True
        return False

    def negative_max(self, depth, alpha, beta):
        """
        使用α-β剪枝的负值极大算法
        :param is_ai: 是否为AI
        :param depth: AI搜索深度
        ...
        for next_step in blank_list:
            ...
            if self.color == 1:
                self.white_index.append(next_step)
            else:
                self.black_index.append(next_step)
            self.all_index.append(next_step)

            value = -self.negative_max(depth - 1, -beta, -alpha)
            if self.color == 1:
                self.white_index.remove(next_step)
            else:
                self.black_index.remove(next_step)
            self.all_index.remove(next_step)
    ...
    def get_table_info(self):
        self.all_index = [(i, j) for i in range(len(self.table_2d)) for j in range(len(self.table_2d[0])) if
                          self.table_2d[i][j] != 0]
        self.white_index = [(i, j) for i in range(len(self.table_2d)) for j in range(len(self.table_2d[0])) if
                          self.table_2d[i][j] == 1]
        self.black_index = [(i, j) for i in range(len(self.table_2d)) for j in range(len(self.table_2d[0])) if
                          self.table_2d[i][j] == -1]
    ...
    def xiazi(self, playerjudger, color, step):
        ...
        self.negative_max(self.depth, -999999, 999999)  # 起始最大最小为正负无穷
        print("本次共剪枝次数：" + str(self.cut_count))
        print("本次共搜索次数：" + str(self.search_count))
        return self.next_point[0]+1, self.next_point[1]+1
